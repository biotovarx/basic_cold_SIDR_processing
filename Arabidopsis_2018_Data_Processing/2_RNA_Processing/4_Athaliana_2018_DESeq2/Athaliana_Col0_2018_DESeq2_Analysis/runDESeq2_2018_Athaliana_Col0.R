...
directory <- "../Athaliana_Col0_VS_Pen1_DESeq2_Analysis/Athaliana_Col0_and_Pen1_2018_HTseq/"

## Percent susept values determined by Bill
Athalian_percent_Susept <- read_csv("../../../../ColdSIDR_Resources/Weldon_Data_Excells/AT_ColdSIDR_PercentSucept.csv")

## COR Genes
UpCORGenes <- read_csv("../../../../Thesis_Supporting_Data/Arabidopsis_Supporting_Data/Shi_et_al_2018_All_Upreg_COR_genes_identified_in_Col.csv")
DownCORGenes <- read_csv("../../../../Thesis_Supporting_Data/Arabidopsis_Supporting_Data/Shi_et_al_2018_All_Downreg_COR_genes_identified_in_Col.csv")
AllCORGenes <- c(UpCORGenes$Gene_ID, DownCORGenes$Gene_ID)


pen1Related <- read_tsv("../../../../Thesis_Supporting_Data/Arabidopsis_Supporting_Data/PEN1_Associated_Genes/Pen1AssociatedGenes.tsv")

pen1Outliers <- c("At_Col0_UTC_9_E3_T43_counts.txt", #UTC
...
outLabel <- paste(LOGsampleFiles, "_", shrinkageType, "_Shrink", sep = "")

# Directory to which output will be saved:
output_dir <- paste(getwd(), "/Athaliana_Col0_2018_DESeq_", outLabel, "_", outDate, sep="")
...
logFile <- file.path(output_dir, paste("Analysis_Log_", outDate, ".txt", sep = ''))
  cat("At_Col0_DE_Analysis",file=logFile,sep="\n")
...
## Metadata example: At_Col0_48hpc_7_E3_T2_AF_counts.txt

## Genotype
sampleGenotype <- sub("At_(.*?)_(.*?)_.*","\\1", sampleFiles)
## Treatment
...
## Circadian 
time_clock <- ifelse(grepl("At_.*?_(12hpc|36hpc)_.*", sampleFiles),"Night",  
                     grepl("At_.*?_(UTC|24hpc|48hpc)_.*", sampleFiles, "Day", NA))
...
sampleTable <- left_join(sampleTable, 
                         Athalian_percent_Susept, 
                         by = c("experiment" = "Experiment", "condition" = "Treatment", "genotype" = "Species"))
...
ddsHTSeq <- DESeqDataSetFromHTSeqCount(sampleTable = sampleTable, directory = directory, design = ~ experiment + condition)

## Change condition and experiment to factors and relevel
colData(ddsHTSeq)$condition <- factor(colData(ddsHTSeq)$condition, 
                                      levels = c("UTC", "12hpc", "24hpc", "36hpc", "48hpc"))
colData(ddsHTSeq)$experiment <- factor(colData(ddsHTSeq)$experiment, 
                                       levels = c("E1", "E2", "E3"))
...
cat(paste(c("Genotype: ", levels(ddsHTSeq$genotype)), collapse = " "), sep = "\n", file=logFile,append=TRUE)
...
p <- ggplot() +
  ...
  geom_point(data = dCOL0Mean,
             ...
             size = 4,
             shape = 16) +
  labs(title = paste("Gene:", pickGene, sep = ""), 
       x= "Sample")
