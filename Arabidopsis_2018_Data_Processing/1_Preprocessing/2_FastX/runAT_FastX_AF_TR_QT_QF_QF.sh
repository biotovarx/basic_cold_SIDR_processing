...
callAF="/usr/local/bin/FastX/0.0.13/fastx_artifacts_filter"
...
do
	...
	# Filters sequencing artifact (reads with all but 3 identical bases):
	# ------------------------------------------------------------
	# FastX settings:
	#	"AF" / $callAF -> Artifact Filer
	...
	echo "FastX-AF processing file $file - `date`" >> $log
	$callFX $options -i $infile -o $outfile >> $log
	# ------------------------------------------------------------
	echo "__________________________________________________" >> $log
	echo >> $log
